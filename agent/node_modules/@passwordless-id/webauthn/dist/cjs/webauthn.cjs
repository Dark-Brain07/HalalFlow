...
async function register(options) {
  ...
  const creationOptions = {
    ...
    pubKeyCredParams: [
      ...
      // RS256 (for older Windows Hello and others)
    ],
    ...
    authenticatorSelection: {
      ...
      // see https://developer.mozilla.org/en-US/docs/Web/API/PublicKeyCredentialCreationOptions#residentkey
  ...
  const json = {
    ...
    // Same as ID, but useful in tests
...
// src/authenticatorMetadata.ts
var authenticatorMetadata = {
...
function toRegistrationInfo(registrationJson, authenticator) {
  ...
  return {
    ...
    // That's specific to this library
...
async function verifyRegistration(registrationJson, expected) {
  ...
  return parsers_exports.toRegistrationInfo(registrationJson, authenticator);
}
...
async function verifySignature({ algorithm, publicKey, authenticatorData, clientData, signature, verbose }) {
  ...
  const algoParams = getAlgoParams(algorithm);
  const isValid2 = await crypto.subtle.verify(algoParams, cryptoKey, signatureBuffer, comboBuffer);
  return isValid2;
}
...
// src/index.ts
var webauthn = { client: client_exports, server: server_exports, parsers: parsers_exports, utils: utils_exports, authenticatorMetadata };
var index_default = webauthn;
